	...
	with open('emloyee_list.csv','r') as f:
		...
		if fact == False:
			for i in employees_details:
				print('\t',i[0].ljust(15),'|  ',i[1].ljust(20),'|  ',i[2].ljust(25))
				if flag == 0:
					print('\t','_'*60)
				print()
				flag += 1
			...
			for i in employees_details:
				print(i[0].ljust(12),'|  ',i[1].ljust(15),'|  ',i[2].ljust(23),'|  ',i[3].ljust(15),'|  ',i[4].ljust(10),'|  ',i[5].ljust(18),'|  ',i[6].ljust(20),)
				if flag == 0:
					print('_'*cols)
				flag += 1
			print()
			...
			if go_back.lower() == 'q':
				...
				print('\n')
				val = input('\t\tEnter the Employee Id that you want to change ')
				for i in employees_details:
					if i[0] == val:
						adm = Person('admin_chng',i[0],i[1],i[2],i[3],i[4],i[5],i[6])
						break
		...
		if name.lower() == 'employee':
			for i in details:
				if id_ == i[0]:
					if pwd_ == i[6]:
						flag = 0
						emp = Person(name,i[0],i[1],i[2],i[3],i[4],i[5],i[6])
						break
					else:
						flag = 0
						print()
			...
			if details[1][0] == id_:
				if details[1][6] == pwd_:
					admin = Person(name,details[1][0],details[1][1],details[1][2],details[1][3],details[1][4],details[1][5],details[1][6])
				else:
...
class Person:
	...
	def employee_in(self,kota):
		...
		with open('emloyee_list.csv','r') as f:
			...
			print('\t',details[0][0].ljust(self.Id_len),'|  ', details[0][1].ljust(self.Name_len),'|  ', details[0][2].ljust(self.Desg_len),'|  ', details[0][3].ljust(self.Joining_len),'|  ', details[0][4].ljust(self.Salary_len) ,'|  ', details[0][5].ljust(self.Project_len),'|  ', details[0][6].ljust(self.Password_len))

			print('_'*cols,'\n')

			for i in range(len(details)):
				if details[i][0] == self.Id:
					print('\t',details[i][0].ljust(self.Id_len),'|  ', details[i][1].ljust(self.Name_len),'|  ', details[i][2].ljust(self.Desg_len),'|  ', details[i][3].ljust(self.Joining_len),'|  ', details[i][4].ljust(self.Salary_len) ,'|  ', details[i][5].ljust(self.Project_len),'|  ', details[i][6].ljust(self.Password_len))
					break
		print('\n\n')
		time.sleep(2)
		...
		if y.lower() == 'q':
			...
			if kota == 'emp':
				print('\n\n')
				print('\tYou Dont have the Admin permission, You can only change your Id Password and Project Working\n')
		...
		if para == self.Password:
			...
			if val == valu:
				...
				with open('emloyee_list.csv','r') as f:
					...
					with open('emloyee_list.csv','w') as f:
						f_writer = csv.writer(f)
						f_writer.writerows(details)

					print('\n\n')
					a = input("Id Password has been Changed !          (Press Enter to Continue)".center(cols))
			...
			with open('emloyee_list.csv','r') as f:
				...
				print('\n\n')
				a = input("New Data has been Updated!                Press Enter to Continue.".center(cols))
	...
	def add_row(self):
		...
		with open('emloyee_list.csv','r') as f:
			...
			with open('emloyee_list.csv','a') as j:
	...
	def del_row(self):
		...
		print('-'*cols)
		print('|','Welcome Administrator to the Employee Record Delete Wizard'.center(cols-4),'|')
		print('-'*cols)
		...
		print('\n\n')
		print('Are you sure you want to delete the Record of Employee with the Id {}'.format(inf).center(cols))
		print('\n')
		...
		if key.lower() == 'y':
			...
			with open('emloyee_list.csv','r') as f:
				...
				for i in range(len(det)):
					...
					time.sleep(3)
					return 1
				with open('emloyee_list.csv','w') as j:
